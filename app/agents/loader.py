...
_EFFICIENCY_RULES = (
    ...
    "- Объединяй независимые проверки в один вызов: если у тебя есть команды хоста, "
    "несколько read-only команд можно выполнить разом через `sh -c 'A && B && C'` "
    "(пайпы, циклы, 2>/dev/null допустимы — это остаётся без подтверждения, если все "
    "команды внутри read-only).\n"
