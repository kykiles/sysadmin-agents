...
class MarkStepParams(BaseModel):
    ...
    status: Literal["done", "failed", "skipped"] = Field(
        description="done — выполнен, в том числе когда проверил и ничего не нашёл: "
                    "отрицательный ответ — это результат, а не провал; "
                    "failed — не смог выполнить; "
                    "skipped — не понадобился: выяснилось, что делать по нему нечего")
...
def build_director_prompt(available_skills: dict[str, str] | None = None,
    ...
    return (
        ...
        "- Табличные данные (однотипные записи, сравнение по столбцам) — таблицей "
        "markdown: Telegram её рисует.\n"
        "- Отдельные пары «ключ: значение» — блоком цитаты (каждая строка начинается "
        "с «> ») или списком, как читается лучше.\n"
        ...
        "у него нет.\n"
        "Когда писать через remember_fact: агент добыл то, что понадобится в следующей "
        "задаче, а в оглавлении этого нет, — топология, пути, версии, принятые решения, "
        "договорённости. Например, агент выяснил, в каком контейнере база и какие в ней "
        "таблицы, или где лежит нужный лог, — запиши до ответа пользователю, иначе "
        "следующий агент будет искать это заново. Разовые находки («что было в этих "
        "логах», «почему упал этот запрос») не пиши: они уходят пользователю ответом.\n"
...
def _steps_block(steps: dict[int, str]) -> str:
    ...
    return (
        ...
        "ходе, что и другие вызовы. Не смог — \"failed\", не понадобился — \"skipped\". "
        "Отмечай по порядку и только сделанное: если пользователь отказал в подтверждении, "
        "пункт не выполнен. Проверил и не нашёл — это \"done\": отрицательный ответ и есть "
        "результат пункта. Если из него следует, что дальше делать нечего, остальные "
        "пункты — \"skipped\", а не \"failed\"."
    )
...
class Director(Agent):
    def __init__(self, llm: LLMClient, gateway=None,
        ...
        async def _spawn(role: str, skills: list[str], task: str, steps: list[int] | None = None) -> dict:
            ...
            # Напоминание в результате, а не только в промпте: в этот момент добытое агентом
            # перед глазами у модели, а блок про память — на одиннадцатом килосимволе.
