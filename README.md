...
3. Пока задача идёт, бот держит TODO-лист одним сообщением: пункты плана пишет Директор и раздаёт агентам, выполнение (✅ ❌ ➖) отмечает сам агент, а «в работе», «ждёт вашего подтверждения» и ❌ после «Нет» ставит код. Неотмеченное к концу задачи — ➖
