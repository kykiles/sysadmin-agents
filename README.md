...
- **Масштабируемость** — новая возможность = один навык, без правок кода и без заведения агентов. Удачный ход задачи Директор сам сохраняет плейбуком через `write_skill` (с подтверждением) — на том (`LEARNED_SKILLS_DIR`), рядом с памятью, а не в каталог образа; прежняя версия при перезаписи уходит в `.history/`
...
| `LEARNED_SKILLS_DIR` | `/data/skills` | Плейбуки, которые записал `write_skill`: только текст (без `metadata`, кода и `scripts/`), имена из `skills/` не перекрывают. Прежние версии — в `.history/<имя>/` |
