...
Команды бота: `/start`, `/help`, `/reset` — очистить историю диалога, `/learn` — самопроверка (давно не подтверждавшиеся знания, факты из карантина и предложения записать новое — всё кнопками), `/reload` — перечитать навыки, `/trace [N]` — ход задачи файлом.
