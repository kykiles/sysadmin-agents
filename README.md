...
- **Опасные операции** (мутации: `restart`, `stop`, `start`, `exec`, `compose up/down`, `shell_exec`, `docker_query`) — требуют подтверждения через inline-кнопки в Telegram (таймаут 5 минут, авто-отклонение). Кнопка **Yes to all** разрешает тот же инструмент на ту же цель до конца ответа
...
3. Для опасных операций бот пришлёт точный вызов и кнопки **Yes / Yes to all / No**. **Yes to all** разрешает тот же инструмент на ту же цель (контейнер, хост, скрипт) до конца ответа; для `sh`, `bash`, `python` и вызовов без явной цели этой кнопки нет
