...
- `log_stats` — распределение значений по файлам логов: фильтр, временное окно и счёт
  делает код, а не ты. Один вызов вместо серии `grep`.
...
### Плейбук: «посчитать по логу»
Считать перебором `grep` не нужно и не надо: пайпов нет, и на подборе шаблонов
вслепую сгорают десятки вызовов. Для этого есть `log_stats`.
1. Найди сам файл (`docker_ps`, затем `ls`/`find` по каталогу проекта) и вызови
   `log_stats` с грубым `extract` — хоть `.` . В ответе придёт `sample`:
   настоящая строка лога. Шаблон пиши по ней, а не по памяти.
2. Распределение: `extract` — то, что считаем (`"status":([0-9]+)`, `"Referer":"https?://([^/"]+)`).
   Группа — значение; без группы считается всё совпадение.
3. Окно по времени — это `window`: шаблон с одной группой-числом и границы
   (`{"pattern": "\"ts\":([0-9.]+)", "min": 1789917899, "max": 1790004299}`).
   Диапазон чисел регулярным выражением не выражают — для этого и есть `window`.
4. Дополнительный отбор строк — `where` (например, только `POST /api`).
   Ротация: окно задевает и текущий файл, и `.1` — передай оба в `paths`, счёт общий;
   архивы `.gz` читаются как есть, распаковывать не нужно.
5. `matched: 0` или `extracted: 0` — не перебирай экранирование, посмотри `sample`
   и `note`: там ровно та строка, на которой шаблон не сошёлся.
6. Сотня-другая строк — просто прочитай их (`tail -n 200`), это короче любого счёта.
